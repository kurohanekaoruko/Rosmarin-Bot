...
- **基本房间运营**
    ...
    - 自动升级、自动建造、自动维修
    - 根据预设布局放置工地
- **任务模块**: 使用任务池来管理Creep所进行的工作 (目前只有搬运、建造、维修、孵化使用了任务模块)。
- **外矿采集**: 从外部房间进行能量采集, 也支持对中央九房的采集。
- **过道采集**: 自动监控并采集过道房间的 PowerBank 和 Deposit。
- **Lab合成**: 自动化的化合物合成, 以及由任务控制的 Boost。
- **工厂生产**: 自动化的商品生产, 搭配资源管理可以实现各个等级商品的生产线。
- **资源管理**: 根据设置的阈值来转移各个房间的资源。
