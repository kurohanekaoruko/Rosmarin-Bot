...
const RoleSpawnCheck = {
    'harvester': (room: Room, current: number, num: number) => {
    ...
    'upgrader': (room: Room, current: number, num: number) => {
        if (room.memory.defend) return false;
    ...
    'transport': (room: Room, current: number, num: number) => {
    ...
    'manager': (room: Room, current: number, num: number) => {
    ...
    'carrier': (room: Room, current: number, num: number) => {
    ...
    'worker': (room: Room, current: number, num: number) => {
        if (room.memory.defend) return false;
    ...
    'mineral': (room: Room, current: number, num: number) => {
        const lv = room.level;
        // 当前等级的最大ext能量容量
    ...
    'universal': (room: Room, current: number, num: number) => {
        const lv = room.level;
        return current < 2 && lv < 3 && (!room.container || room.container.length < 1);
    },
    'UP-upgrade': (room: Room, current: number, num: number) => {
    ...
    'UP-repair': (room: Room, current: number, num: number) => {
        // 加速刷墙
