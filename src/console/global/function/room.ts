...
export default {
    room: {
        ...
        info(roomName?: string) {
            ...
            str += '<th style="text-align: center;"> Storage </th>'
            str += '<th style="text-align: center;"> Terminal </th>'
...
const showRoomInfo = function (roomName: string) {
    ...
    if (room.storage) {
        ...
        if (store < 0.8) {
            str += `<td>${colorText(`${UsedCapacity.toFixed(2)}M/${Capacity.toFixed(2)}M`, '#62BE78')}</td>`;
        } else if (store < 1) {
            str += `<td>${colorText(`${UsedCapacity.toFixed(2)}M/${Capacity.toFixed(2)}M`, '#D9C07B')}</td>`;
        } else {
            str += `<td>${colorText(`${UsedCapacity.toFixed(2)}M/${Capacity.toFixed(2)}M`, '#EF4E4E')}</td>`;
        }
    } else {
        str += `<td>${colorText('未建造', '#8E8E8E')}</td>`;
    }
    ...
    if (room.terminal) {
        ...
        if (store < 0.8) {
            str += `<td>${colorText(`${(store*100).toFixed(2)}%`, '#62BE78')}</td>`;
        } else if (store < 1) {
            str += `<td>${colorText(`${(store*100).toFixed(2)}%`, '#D9C07B')}</td>`;
        } else {
            str += `<td>${colorText(`${(store*100).toFixed(2)}%`, '#EF4E4E')}</td>`;
        }
    } else {
        str += `<td>${colorText('未建造', '#8E8E8E')}</td>`;
    }
